...
const notificationMapping: Record<string, NotificationConfigInterface> = {
  'customer.update': {
    roles: ['financial-advisor', 'insurance-agent', 'banking-service-provider'],
    key: 'financial-service-assigned',
    tenantPath: ['financial_service', 'customer'],
    userPath: [],
  },
  'insurance_policy.update': {
    roles: ['financial-advisor', 'insurance-agent'],
    key: 'insurance-policy-updated',
    tenantPath: ['financial_service', 'insurance_policy'],
    userPath: [],
  },
  'banking_service.update': {
    roles: ['financial-advisor', 'banking-service-provider'],
    key: 'banking-service-updated',
    tenantPath: ['financial_service', 'banking_service'],
    userPath: [],
  },
  'nps.update': {
    roles: ['financial-advisor', 'customer'],
    key: 'nps-score-updated',
    tenantPath: ['financial_service', 'nps'],
    userPath: [],
  },
};

const ownerRoles: string[] = ['financial-advisor'];
const customerRoles: string[] = ['customer'];
const tenantRoles: string[] = ['financial-advisor', 'insurance-agent', 'banking-service-provider'];
...
export async function notificationHandlerMiddleware(req: NextApiRequest, entityId: string) {
  ...
  const tenant = await prisma.financial_service.findFirst({
