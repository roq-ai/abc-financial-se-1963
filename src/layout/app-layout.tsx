...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['financial-advisor', 'insurance-agent', 'banking-service-provider'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<FinancialServiceInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/financial-services` : null),
    () => getFinancialServices({ tenant_id: session?.user?.tenantId }).then(({ data }) => data),
  );
  loading = isLoading;
  ...
  const MockedLinkItems: Array<NavItemPropsInterface> = [
...

    {
      ...
      path: isTenantUser ? `/financial-services/view/${data?.[0]?.id}` : '/financial-services',
    ...
    { name: 'Nps', path: '/nps', entity: 'nps', service: AccessServiceEnum.PROJECT, icon: FiStar },
