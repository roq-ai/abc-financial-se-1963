...
import { NpsListPage as ShowCaseList } from 'pages/nps';

function DashboardPage() {
  const { session } = useSession();

  const bankingServiceStat = useSWR<PaginatedInterface<BankingServiceInterface>>(
    () => `/banking-services`,
    () => getBankingServices({ limit: 1, offset: 0 }),
  );

  const insurancePolicyStat = useSWR<PaginatedInterface<InsurancePolicyInterface>>(
    () => `/insurance-policies`,
    () => getInsurancePolicies({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      ...
      result: bankingServiceStat?.data?.totalCount,
      isLoading: bankingServiceStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Insurance Policies',
      result: insurancePolicyStat?.data?.totalCount,
      isLoading: insurancePolicyStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Customers',
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`nps`} passHref legacyBehavior>
                  <Button
