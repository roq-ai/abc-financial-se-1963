...
export default RoqAuth({
  hooks: {
    ...
    onRegisterSuccess: async ({ user }) => {
      ...
      if (user.roles.data.some((role) => ['Financial Advisor'].includes(role.name))) {
        const tenant = await roqClient.asSuperAdmin().tenant({ id: user.tenantId });
        const { name: tenantName } = tenant.tenant ?? {};
        seedNumber = await prisma.financial_service.count();
        const mainEntity = await prisma.financial_service.create({
          data: {
            tenant_id: user.tenantId,
            user_id: owner.id,
            name: tenantName,
